...
class KernelSimpleEnergyClassify(object) :
	...
	def gps_predict_label(self, set_of_gps_points):
		if len(set_of_gps_points) < 2:
			...
			return	(Distribution(5,[0.25, 0.25, 0.25, 0, 0.25]), 0)
		...
		if total_distance > 4500 * 2 : # this guy is crazy
			...
			denominator = sum(likelihood) - likelihood[3]
			for label in range(0,5) :
				if sum(likelihood) > 0:
					if label == 3:
						posterior_pmf[label] = 0
					else:
						posterior_pmf[label]=likelihood[label]/denominator
				else:
					posterior_pmf[label] = 0.25
			...
			if (self.use_gps == 1):
				if should_i_trust_gps == 1: 
					if (gps_posterior_dist.mode() == 0 or gps_posterior_dist.mode() == 4): 	
						#prev_gps_pred = gps_posterior_dist.mode()
