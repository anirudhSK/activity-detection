...
class MarkovChain(object) :
	...
	def simulate(self,duration) :
		...
		state = min(int(random() * 5),5) # sample between 0 and 4 uniformly
		sampled_dtmc=[]
		while ( time < duration ) :
			...
			next_state=min(int(random() * 4),4) # sample between 0 and 3 uniformly
			target_state=(state+1+next_state)%5 # cyclic addition
