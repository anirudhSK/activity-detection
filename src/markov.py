...
class MarkovChain(object) :
	...
	def simulate(self,duration) :
		...
		state = min(int(random() * len(states)),len(states)) # sample between 0 and 4 uniformly
		sampled_dtmc=[]
		while ( time < duration ) :
			...
			next_state=min(int(random() * (len(states)-1)),(len(states)-1)) # sample between 0 and 3 uniformly
			target_state=(state+1+next_state)%len(states) # cyclic addition
